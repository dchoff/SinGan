...
parser.add_argument('--inj_scale', default='0', type=int, help='Desired injection scale for image')
...
def main():
    ...
    subprocess.call(['python', './cartoonGAN/test.py', '--input_dir', args.res_dir, '--gpu', '0', '--mod_name', args.model_name])
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    args.num_scale = int(np.round(np.log(tmp_scale) / np.log(scale_factor)))-args.inj_scale
    args.size_list = [int(args.img_size_min * scale_factor**(i+args.inj_scale)) for i in range(args.num_scale + 1)]
    # original_scales = args.size_list
    # args.size_list = args.size_list[:-inj] # check for different scale injection
    # args.num_scalee = args.num_scale - inj


    discriminator = Discriminator()
    generator = Generator(args.img_size_min, args.num_scale, scale_factor, args.inj_scale)
